...
| | |\
| |**声音复刻 ICL2.0场景：** |\
| |使用非中英文语种时，必须指定明确语种，并且合成文本也必须是指定语种的文本，比如日语等； |\
...
| |取值示例：如在一通电话中的多次 TTS 请求，建议为该通电话使用 UUID 生成一个唯一的 session_id，并在所有 TTS 请求中传递相同的 session_id。 |\
